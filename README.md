...
<!---
Latino9010/Latino9010 is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
...
bool IsMarketOpen(const string symbol, const bool debug = false) {
    ...
    while(session > -1)
    {
        if(SymbolInfoSessionTrade(symbol,day_of_week,session,from,to ))
        {
            ...
            const int sessionFrom = from; // (int) MathMod(from, PeriodSeconds(PERIOD_D1));
            const int sessionTo = to; //(int) MathMod(to, PeriodSeconds(PERIOD_D1));
